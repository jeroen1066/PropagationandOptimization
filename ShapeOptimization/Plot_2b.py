...
plot_tuned_results = True

if plot_tuned_results:
    fig, axs = plt.subplots(2,2, figsize=(8,6))
    for i in range(len(seeds_to_test)):
        data_file = save_directory + 'maco_tuned_seed_' + str(seeds_to_test[i]) + '.dat'
        ancillary_file = save_directory + 'maco_tuned_seed_' + str(seeds_to_test[i]) + '_ancillary.txt'
        file = open(data_file,'rb')
        data = pickle.load(file)[0]
        file.close()
        x = data[0]
        ...
        y_per_gen = np.array(data[2])
        # plot 1
        axs[(0, 0)].scatter(np.asarray(y)[:,0], np.asarray(y)[:,1], label=str(seeds_to_test[i]) + ' tuned')
        axs[(0, 0)].set_ylim([0, 8])
        axs[(0, 0)].set_xlim([0, 8])
        axs[(0, 0)].grid()
        axs[(0, 0)].set_xlabel('mass fitness')
        ...
        axs[(0, 0)].legend()
        # plot 2
        axs[(0, 1)].scatter(np.asarray(y)[:, 0], np.asarray(y)[:, 2], label=str(seeds_to_test[i]) + ' tuned')
        axs[(0, 1)].set_ylim([0, 8])
        axs[(0, 1)].set_xlim([0, 8])
        axs[(0, 1)].grid()
        axs[(0, 1)].set_xlabel('mass fitness')
        ...
        axs[(0, 1)].legend()
        # plot 3
        axs[(1, 0)].scatter(np.asarray(y)[:, 1], np.asarray(y)[:, 2], label=str(seeds_to_test[i]) + ' tuned')
        axs[(1, 0)].set_ylim([0, 8])
        axs[(1, 0)].set_xlim([0, 8])
        axs[(1, 0)].grid()
        axs[(1, 0)].set_xlabel('l/d fitness')
