...
class CapsuleEntryProblem:
    ...
    def fitness(self, parameters):
        ...
        fitness = np.array([1e5 / mass_capsule])
        if not succesfull_completion or has_skipped:
            fitness = np.array([1e10])
        ...
        if max_g_load > self.constraints[2]:
            fitness *= max_g_load / self.constraints[2] * 100
    ...
    def optimize(self, numpops: int, numgens: int, numrepeats: int, seeds: list[float], start_time: float) -> None:
        ...
        for i in range(numrepeats):
            ...
            if self.optimizer == pg.maco:
                ...
                algo = pg.algorithm(self.optimizer(selection='truncated',seed=seed,crossover='sbx',mutation='polynomial'))
